...
%%% Better titles?
%%% Verifying Consistency Mechanism for Declared Memory Access in Heterogeneous Systems: the VectorPU Approach
% Verifying Memory Consistency for Programs with Declared Memory Access in Heterogeneous Systems 
%%%  An effect system for Memory Access Declarations
%\author{Ludovic Henrio, Christoph Kessler,  Lu Li\thanks{L. Henrio is with the 
%  E-mail: 
%\newline
%C. Kessler and L. Li are with the . Email: 
%firstname.lastname@liu.se}\vspace{-3ex}}
\author{\IEEEauthorblockN{Ludovic Henrio}
\IEEEauthorblockA{Universit\'e~C\^ote~d'Azur, CNRS, I3S, France.\\ludovic.henrio@cnrs.fr
}
\and
\IEEEauthorblockN{Christoph Kessler and Lu Li}
\IEEEauthorblockA{University of Linköping, Sweden\\
...
\end{abstract}
\begin{IEEEkeywords}
Memory consistency, CPU-GPU heterogeneous systems, 
data transfer, software caching, cache coherence 
\end{IEEEkeywords}
...
%reason on  correct memory access  
%in presence of cache, specification of a cache update/invalidation mechanism
%
%Formal Verification of Coherence for a Shared Memory Multiprocessor Model
%specification in CCS of a cache coherence protocol and shows that it verifies 
%correctness 
%properties expressed in mu calculus 
%
%several works on consistency+WMM
%
%
%
%Not found anything on abstract information on coherency and use in triggering 
%consistence 
%exchanges.
%
%Verification techniques for cache coherence protocols -- a bit old
%
%A Maude Framework for Cache Coherent
%Multicore Architectures -->  ``As a step towards understanding how the data
%organisation impacts the performance of a given parallel program using
%shared memory, this paper proposes a framework defined in Maude for
%the executable modelling of program execution on cache coherent multi-
%core architectures, formalising the interactions between cores executing
%tasks, their caches, and main memory.'' 
%
%An Operational Semantics of
%Cache Coherent Multicore Architectures
\AtEndEnvironment{thebibliography}{
%\input{references.tex} - entries moved to bib-file
}
%%
%%    
...
A simplified model where we have one single variable (could be a complex structure or an array), and 2 memory locations, they are denoted "local" and "remote".


 \[S= Push | Pull | R| W | RW | remR| remW | remRW | \noop | S;S'\] (RW is the same as 
 may write) rem is for remote operations; Pull could be remPush but it sounds strange
Effects are I or V, X,Y,Z,T are effect variables.


We could add an instruction to change execution context (eg evaluate a computation on a gpu) ; it would witch the validity pair and continue execution. This could be designed as rem(S) in the syntax. Is it good or not?

variables are universally quantified in each typing rule.

We suppose the program has a single variable, extension to multiple variables is easy but requires additional notations and constructs ... to see
\begin{mathpar}
Push: (V,X)\to (V,V)
