...
\TODO{improve the link to skePU (here or additional section earlier?)}
...
First,  we suppose here that memory transfer operations are performed conveniently and that if several push/pull are performed on overlapping memory locations, the transfer is done in a correct way. \TODO{write properly, I mean : }When we write $\Push\ x; \Push\ y$ on two overlapping arrays it pushes two overlapping arrays.
...
Note that the application of these rules terminate provided the number of arrays is finite.
