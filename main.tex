...
First,  we suppose here that memory transfer operations are performed addording to the following rules. If several push/pull are performed on overlapping memory locations, the transfer is done in a correct way. \TODO{write properly, I mean : }When we write $\Push\ x; \Push\ y$ on two overlapping arrays it pushes two overlapping arrays. 
\TODO{The point is that the common elements
in x and y are replicated in the two device copies of x and y.}
\TODO{Really? meaning you cannot have neither W nor RW on x or y? 
this is really restrictive!!!}
\TODO{to remove the restriction you need to create overlapping copies on the other side ... or completely change the mechanism}
