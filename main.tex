...
First, in VectorPU no memory allocation occurs during data transfer (memory is pre-allocated).\TODO{To be more precise (see also my added text at the end of Sect. 2): The first pvector on a vector passed as argument for access on device will (over)allocate space for the entire vector, all subsequent pvector accesses to the same vector can skip the allocation.} Consequently, two successive data transfers of the same memory location will be written to the same memory location, even if the two initial locations are accessed through different (overlapping) vectors.
...
\TODO{rewrite: the first part is in fact not much different from the previous section}
