...
%% \address[label1]{}
%% \address[label2]{}
\author[lip]{Ludovic Henrio\fnref{cor2}\corref{cor1}}
...
\fntext[cor2]{This work has been mostly founded and realised while Ludovic Henrio was affiliated to: Universit\'e~C\^ote~d'Azur, CNRS, I3S, France.}
...
$\oplus$\footnote{We  suppose  that evaluation of $\oplus$ always succeed, and in particular  variables accessed by the operation are specified as a $r$ operation preceding the condition.}. The next rule applies an effect of the operation $E$ on a variable $x$ updating the validity store, 
...
%From a theoretical point of view,  it is possible to define an abstraction such that for example $\abs x=\abs y$ but then the definition of the mapping would be very difficult to use.
...
a well-declared program encoding this behaviour would be:\\
$\begin{array}{l}
RW~\abs x\{w~x\};\\
\rem{R~\abs x}\{\rem{r~x}\}
\end{array}$\\
...
This time and space could alternatively be saved if one had a precise enough static analysis that could infer potential overlapping between arrays, choosing a precise enough static analysis, implementing it or designing a dedicated one is outside the scope of this paper.
